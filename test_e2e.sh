...
export CLASSPATH="/usr/share/openjfx/lib/*:${CLASSPATH}"

# Test 1: TheMovieDB
echo "Testing TheMovieDB..."
java -cp "dist/FileBot_4.8.0.jar:/usr/share/openjfx/lib/*" -Djna.nounpack=false -Djna.library.path=/usr/lib/x86_64-linux-gnu/ -Djava.library.path=lib/native/linux-amd64 net.filebot.Main -rename test_env/dummy/Avatar.2009.mp4 --db TheMovieDB -non-strict --log ALL --q "Avatar 2009"
...
java -cp "dist/FileBot_4.8.0.jar:/usr/share/openjfx/lib/*" -Djna.nounpack=false -Djna.library.path=/usr/lib/x86_64-linux-gnu/ -Djava.library.path=lib/native/linux-amd64 net.filebot.Main -rename test_env/dummy/The.Simpsons.S01E01.mp4 --db TheTVDB -non-strict --log ALL --q "The Simpsons"
